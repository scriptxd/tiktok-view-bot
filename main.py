        ...
        try:
            ...
            try:
                if response.json()['status_code'] == 0:
                    _lock.acquire()
                    print(Colorate.Horizontal(Colors.purple_to_red, f'TikTok Viewbot by Tunay ^| success: {success} fails: {fails} reqs: {reqs} rps: {rps} rpm: {rpm}'))
                    success += 1
                    _lock.release()
            except:
...
def fetch_proxies():
    ...
    for url in url_list :
        ...
        if response.ok:
            with open("C:/Users/MSI/Desktop/tiktok/proxies.txt", "a+") as f:
...
if __name__ == "__main__":
    with open('C:/Users/MSI/Desktop/tiktok/devices.txt', 'r') as f:
        devices = f.read().splitlines()
    
    with open('C:/Users/MSI/Desktop/tiktok/config.json', 'r') as f:
        config = json.load(f)
    ...
    set_title("Tunay Tool")
    txt = """\n\n
    TikTok Viewbot by ig: @birfrontendci \n"""
    print(
        Colorate.Vertical(
